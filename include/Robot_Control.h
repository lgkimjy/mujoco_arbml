...
public:
	...
	//////////	Functions	//////////
	void initParameters(const mjModel* model_mj, CARBML& robot);
	void initEEParameters(const mjModel* model, CARBML& robot);
	void initCtrlParameters(const mjModel* model_mj, CARBML& robot);
	void outputEEInformation();

	void UserControl(CARBML& robot, mjModel* uModel, mjData* uData);
	void getFeedbackInformation(const mjData* data, CARBML& robot);
	void computeControlInput(CARBML& robot);

	void computeCoMMotion(CARBML& robot);
	void computeEEKinematics(CARBML& robot, Eigen::Matrix<double, TOTAL_DOF, 1>& xidot);
	void computeLinkKinematics(CARBML& robot);	//	Compute kinematics and Jacobian, Jdot, etc

	//	Check code validaty
	void compareModelComputation(const mjModel* model, mjData* data, CARBML& robot, const int& count);
};
