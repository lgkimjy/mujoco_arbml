...
////////////////////////////////////////////////////////////////////////////////
void CRobotControl::initParameters(const mjModel* model_mj, CARBML& robot)
{
	/////	Set joint range of motion, end-effector, etc
	initEEParameters(model_mj, robot);

	/////	Set controller parameters
	initCtrlParameters(model_mj, robot);
...
////////////////////////////////////////////////////////////////////////////////
void CRobotControl::initEEParameters(const mjModel* model, CARBML& robot)
{
	int i;
...
void CRobotControl::initCtrlParameters(const mjModel* model_mj, CARBML& robot)
{
...
//	USER Control Core !!
////////////////////////////////////////////////////////////////////////////////
void CRobotControl::UserControl(CARBML& robot, mjModel* model, mjData* data)
{
	int i;
	...
	if (count_sim > 0)	compareModelComputation(model, data, robot, count_ctrl);
	...
	if (count_sim % CONTROL_RATE == 0) {
		...
		// cout << "Exec Time : " << std::fixed << DefaultSec.count() << " [s] to run" << endl;
...
///////////////////////////////////////////////////////////////////////////
void CRobotControl::getFeedbackInformation(const mjData* data, CARBML& robot)
{
...
void CRobotControl::computeControlInput(CARBML& robot)
{
...
////////////////////////////////////////////////////////////////////////////////
void CRobotControl::computeLinkKinematics(CARBML& robot)
{
...
void CRobotControl::computeCoMMotion(CARBML& robot)
{
	///	Time Stamp Routine : Start
	std::chrono::system_clock::time_point StartTime = std::chrono::system_clock::now();
	...
	std::chrono::microseconds micro = std::chrono::duration_cast<chrono::microseconds>(EndTime - StartTime);
	//std::chrono::milliseconds mill = std::chrono::duration_cast<chrono::milliseconds>(EndTime - StartTime);
	//std::chrono::seconds sec = std::chrono::duration_cast<chrono::seconds>(EndTime - StartTime);

	std::cout.precision(6);
	//cout << "Exec Time : " << std::fixed << DefaultSec.count() << " [s] to run" << endl;
	//cout << "Exec Time : " << std::fixed << nano.count() << " [ns] to run" << endl;
	cout << "Exec Time 2 : " << std::fixed << micro.count() / 20. << " [us] to run" << endl;
	//cout << "Exec Time : " << std::fixed << mill.count() << " [ms] to run" << endl;
	//cout << "Exec Time : " << std::fixed << sec.count() << " [s] to run" << endl;
}
...
////////////////////////////////////////////////////////////////////////////////
void CRobotControl::computeEEKinematics(CARBML& robot, Eigen::Matrix<double, TOTAL_DOF, 1>& xidot)
{
	int i, j, k;
...
void CRobotControl::compareModelComputation(const mjModel* uModel, mjData* uData, CARBML& robot, const int& count)
{
	int i, j, k;
	int error = 0;
	int mjBodyID;
