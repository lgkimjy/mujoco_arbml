...
mjModel* LoadModel(const char* file, mj::Simulate& sim) {
  ...
  ///==================== Modified Code : By Y.Oh ===================////
	mjModel* mnew = mj_loadXML(filename, nullptr, loadError, mj::Simulate::kMaxFilenameLength);

	//	Remove trailing newline character from loadError
	if (loadError[0]) {
		size_t error_length = mju::strlen_arr(loadError);
		if (loadError[error_length - 1] == '\n') {
			loadError[error_length - 1] = '\0';
		}
	}
  ...
  /////////////////////////// USER CODE : START ///////////////////////////
	if(strstr(filename, ".xml") != nullptr) {
		robot.initRobot(mnew);
		robot_control.initParameters(mnew, robot);
	}

	//if (del_T != mnew->opt.timestep) {
	//	_ErrorMsg("Check Sampling Time in RobotConfig.h !!!");
	//}
	//////////////////////////// USER CODE : END ////////////////////////////
      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          if (elapsedSim < 0 || elapsedCPU < 0 || syncCPU == 0 || misaligned || sim.speedChanged) {
            ...
            ////////////////// USER CODE : START //////////////////
						robot_control.UserControl(robot, m, d);
						/////////////////// USER CODE : END ///////////////////
...
///////////////////////////// USER CODE : START /////////////////////////////
#if defined(ROBOT1)
	const char* filename = "../model/DualArm3DoF_FloatingBase.xml";
#elif defined(ROBOT2)
	const char* filename = "../model/DualArm_Torso.xml";
#elif defined(ROBOT3)
	const char* filename = "../model/DualArm.xml";
#elif defined(ROBOT4)
	const char* filename = "../model/Arm-Hand/Arm-Hand.xml";
#elif defined(ROBOT5)
